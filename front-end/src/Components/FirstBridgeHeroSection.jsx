...
import { firstBridgeData } from '../Constants/firstBridgeData';
import { Link } from 'react-router-dom';

const FirstBridgeHeroSection = () => {
  return (
    <>
      <div>
        ...
        <div class="bg-white rounded-[10px] flex justify-between overflow-hidden shadow-md mb-40">
          <Link to="/secondbridge">
            <button className="text-2xl font-medium font-Grotesk text-white bg-[#296B82] py-4 px-14 rounded-[10px]">
              The Bridge
            </button>
          </Link>
          <button className="text-2xl font-medium font-Grotesk text-secondary">
            The Horizon
          </button>
      ...
      <div className="grid grid-cols-2 gap-20 mb-40">
        {firstBridgeData.map(({ id, image, title, description, list }) => (
          <div
...
export default FirstBridgeHeroSection
